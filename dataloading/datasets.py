...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "CTP" not in self.dataset and "PMs" not in self.dataset:
            ...
            lbl_tmp = sitk.GetArrayFromImage(sitk.ReadImage(img_path.split('study_')[0] + 'label_' + img_path.split('study_')[-1] + ".nii.gz"))
...
class TrainDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        if 'CTP' not in args.dataset and "DWI" not in args.dataset and "PMs" not in args.dataset:
            ...
            self.image_dirs = [fold for fold in self.image_dirs if fold.split("study_")[-1][4:10] not in self.test_patients
                               and fold.split("study_")[-1][4:10] not in self.exclude_patients]
        ...
        if "CTP" in args.dataset or "DWI" in args.dataset or "PMs" in args.dataset:
            ...
            val_idx = [idx for idx, fold in enumerate(self.image_dirs) if fold.split("study_")[-1][4:10] in self.val_patients]
            ...
            if "CTP" not in self.dataset and "PMs" not in self.dataset:
                ...
                if "DWI" not in self.dataset: sprvxl = sitk.GetArrayFromImage(sitk.ReadImage(self.sprvxl_dirs[pat_idx]))
                    ...
                    for slice_idx in range(img.shape[0]):
                        ...
                        img_toadd = sitk.GetArrayFromImage(sitk.ReadImage(sprvxl_path))
                        if len(img_toadd.shape)>2: img_toadd = img_toadd[:,:,0]
                        sprvxl[slice_idx, ...] = img_toadd  # sitk.GetArrayFromImage(sitk.ReadImage(sprvxl_path))[:, :, 0]
            else:
                # sample slice idx
